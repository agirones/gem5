#!/bin/bash
# Sensitivity analysis for Sereno/ckpt: sweep broadcastMax (1–5) and
# dependentsThreshold (−1 to 4), submitting one SLURM job per
# (benchmark, simpoint, configuration) triple.
#
# Output tree:
...
SENS_BASE="${GEM5_ROOT}/runs/output/micro26/sereno/ckpt/sensibility_analysis"
SLURM_SCRIPT="${GEM5_ROOT}/slurm/single_simpoint_job.slurm"

echo "=== Sereno sensitivity analysis ==="
echo "  broadcastMax        : 1 .. 5"
echo "  dependentsThreshold : -1 .. 4"
echo "  CPT_BASE            : ${CPT_BASE}"
echo ""

total_jobs=0

for BROADCAST_MAX in $(seq 1 5); do
    for DEPENDENTS_THRESHOLD in $(seq -1 4); do

        OUTPUT_DIR="${SENS_BASE}/bmax${BROADCAST_MAX}_dt${DEPENDENTS_THRESHOLD}"
        mkdir -p "${OUTPUT_DIR}/slurm_logs"

        echo "--- bmax=${BROADCAST_MAX}  dt=${DEPENDENTS_THRESHOLD}  -> ${OUTPUT_DIR}"
