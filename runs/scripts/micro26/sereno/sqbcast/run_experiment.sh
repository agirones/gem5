#!/bin/bash
# Run gem5 simpoint jobs for Sereno v2 (broadcastMax=1, dependentsThreshold=2,
# with squash-induced extra broadcasts — branch 'sereno-mispredict-extra-broadcasts').
#
# This version requires a gem5 binary built from the
# 'sereno-mispredict-extra-broadcasts' branch.  Point GEM5_ROOT_V2 at a
# workspace where that binary has been compiled, or set GEM5_BINARY explicitly.
#
# Usage:
...
#   GEM5_ROOT_V2  – gem5 root containing the v2 binary at build/X86/gem5.opt
...
# If you built the v2 binary in a separate git worktree, set this:
...
OUTPUT_DIR="${GEM5_ROOT}/runs/output/micro26/sereno/sqbcast"
SLURM_SCRIPT="${GEM5_ROOT}/slurm/single_simpoint_job.slurm"

mkdir -p "${OUTPUT_DIR}/slurm_logs"

echo "=== Submitting Sereno v2 jobs (broadcastMax=${BROADCAST_MAX}, dependentsThreshold=${DEPENDENTS_THRESHOLD}, squash-induced broadcasts) ==="
