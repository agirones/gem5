#!/usr/bin/env python3
"""
Violin plot of intFpVecDestRegsPerDispatchedInst for each benchmark
under runs/output/micro26/whisper/1B_2P.

Each violin shows the full distribution (KDE over discrete levels) of the
number of int/fp/vec destination registers per dispatched instruction.
A white dot marks the weighted mean; the thick bar covers the IQR; the thin
whisker covers ±1 std; orange and red ticks mark the 90th and 95th
percentiles.
An extra "Average" violin at the right shows the cross-benchmark aggregate.

Stat name: system.cpu.iew.intFpVecDestRegsPerDispatchedInst
"""
...
STAT_PREFIX = "system.cpu.iew.intFpVecDestRegsPerDispatchedInst"
...
def build_plot_data(
    ...
    def _stats(levels: dict[int, float]):
        ...
        return arr, float(np.mean(arr)), float(np.std(arr))
    ...
    return all_labels, all_samples, np.array(all_means), np.array(all_stds), max_level
...
def _draw_violin(ax, x_pos: float, samples: np.ndarray, mean: float, std: float,
    ...
    tick_hw = width * 0.30
    for pct, pct_color in [(90, 'darkorange'), (95, 'crimson')]:
        pval = np.percentile(samples, pct)
        ax.plot([x_pos - tick_hw, x_pos + tick_hw], [pval, pval],
                color=pct_color, lw=1.5, zorder=6)
...
def plot(
    ...
    ax.set_ylabel("Int/fp/vec destination registers per instruction")
    ax.set_title(
        "Int/Fp/Vec Destination Registers per Dispatched Instruction\n"
    ...
    plt.tight_layout()
    out_path = output_dir / "intfpvec_dest_regs_per_inst_violin.png"
