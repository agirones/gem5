#!/usr/bin/env python3
"""
Violin plot of nonReadySrcOpsDispatchedPerCycle for each benchmark
under runs/output/micro26/whisper/1B_2P.

Each violin shows the full distribution (KDE over discrete levels) of the
fraction of active cycles spent at each non-ready source operand count.
A horizontal white line marks the weighted mean; the thick bar covers
±1 weighted standard deviation; the thin whisker extends to min/max.
...
STAT_PREFIX = "system.cpu.iew.nonReadySrcOpsDispatchedPerCycle"
...
def parse_histogram(stats_file: Path) -> dict[int, float]:
    """
    Parse the nonReadySrcOpsDispatchedPerCycle histogram from a stats.txt.
    ...
    for line in dump.splitlines():
        if not line.startswith(STAT_PREFIX + "::"):
            continue
        # e.g.  system.cpu.iew.nonReadySrcOpsDispatchedPerCycle::3   42   ...
...
def build_plot_data(
    ...
    def _stats(levels: dict[int, float]):
        ...
        # Build a synthetic sample by replicating each level proportionally.
        # 10 000 points gives enough resolution for KDE without being heavy.
        N = 10_000
        pts = []
    ...
    # Average benchmark: merge all levels with equal weight across benchmarks
...
def _draw_violin(ax, x_pos: float, samples: np.ndarray, mean: float, std: float,
    ...
    if len(set(samples)) == 1:
        # Degenerate: all values identical – draw a thin spike
    ...
    density = kde(y_grid)
    # Normalise so the widest point equals `width / 2`
    ...
    ax.plot([x_pos, x_pos], [q25, q75], color='black', lw=4, solid_capstyle='round', zorder=4)
    # ±1 std thin whisker
    ax.plot([x_pos, x_pos], [mean - std, mean + std],
            color='black', lw=1.5, solid_capstyle='round', zorder=4)
    # 90th and 95th percentile tick marks
    tick_hw = width * 0.30   # half-width of the tick dash
    ...
    # Mean dot
    ax.scatter([x_pos], [mean], color='white', edgecolors='black', s=30, zorder=7, linewidths=0.8)
...
def plot(
    ...
    # Colour: benchmarks in steelblue, "Average" in tomato
    ...
    ax.set_ylabel("Non-ready source operands per active cycle")
    ax.set_title("Non-Ready Source Operands Dispatched per Active Cycle\n"
                 "(violin = KDE, bar = IQR, whisker = ±1 std, dot = mean; weighted simpoints)")
    ax.yaxis.grid(True, linestyle='--', alpha=0.5, zorder=0)
    ax.set_axisbelow(True)

    # Legend for percentile markers
    ...
    plt.tight_layout()
    out_path = output_dir / "non_ready_src_ops_dispatched_per_cycle.png"
