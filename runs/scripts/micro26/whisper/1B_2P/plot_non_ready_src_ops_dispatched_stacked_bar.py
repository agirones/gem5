#!/usr/bin/env python3
"""
Stacked bar chart of nonReadySrcOpsDispatchedPerCycle for each benchmark
under runs/output/micro26/whisper/1B_2P.

Each bar shows the fraction of active cycles spent at each non-ready source
operand count (0, 1, 2, …, 19, 20+).  Levels >= 20 are aggregated into a
single "20+" category.
...
# Levels >= this value are folded into a single "20+" bucket.
AGG_THRESHOLD = 20
...
def build_bar_data(
    ...
    Returns:
        ...
        fractions   – 2-D array of shape (n_bins, n_benchmarks+1);
                      each column sums to 1 (fraction of active cycles)
        bin_labels  – label for each row ("0", "1", …, "19", "20+")
    """
    benchmarks = sorted(benchmark_data.keys())

    # Determine the max level observed across all benchmarks.
    max_raw_level = 0
    ...
    # Build bin labels: 0 … agg_threshold-1, then "20+" if needed.
    ...
    # Average benchmark: merge all levels without extra weighting across bms.
    ...
    for col, levels in enumerate(all_data):
        ...
        for row, lbl in enumerate(bin_labels):
            if lbl.endswith("+"):
                # Aggregate all levels >= agg_threshold
                val = sum(
...
def plot(
    ...
    # Build colour map: tab20 for up to 20 discrete levels, gray for "20+".
    cmap = plt.cm.get_cmap('tab20', agg_threshold)
    colors = [cmap(i) for i in range(min(n_bins, agg_threshold))]
    if n_bins > agg_threshold:
        colors.append('dimgray')  # colour for "20+"
    ...
    ax.set_ylabel("Fraction of active cycles")
    ax.set_title(
        "Non-Ready Source Operands Dispatched per Active Cycle\n"
        "(stacked bar, fraction of active cycles; weighted simpoints)"
    )
    ax.yaxis.grid(True, linestyle='--', alpha=0.4, zorder=0)
    ax.set_axisbelow(True)

    # Legend: put it outside the plot on the right.
    ax.legend(
        title="Non-ready\nsrc ops",
    ...
    plt.tight_layout()
    out_path = output_dir / "non_ready_src_ops_dispatched_stacked_bar.png"
