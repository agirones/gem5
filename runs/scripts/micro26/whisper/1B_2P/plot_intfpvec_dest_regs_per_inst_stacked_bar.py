#!/usr/bin/env python3
"""
Stacked bar chart of intFpVecDestRegsPerDispatchedInst for each benchmark
under runs/output/micro26/whisper/1B_2P.

Each bar shows the fraction of dispatched instructions that have 0, 1, 2, …
int/fp/vec destination registers.  Levels >= AGG_THRESHOLD are folded into
a single bucket.
...
def build_bar_data(
    ...
    for col, levels in enumerate(all_data):
        ...
        for row, lbl in enumerate(bin_labels):
            if lbl.endswith("+"):
                val = sum(cnt for lvl, cnt in levels.items() if lvl >= agg_threshold)
            else:
                val = levels.get(int(lbl), 0.0)
...
def plot(
    ...
    plt.tight_layout()
    out_path = output_dir / "intfpvec_dest_regs_per_inst_stacked_bar.png"
