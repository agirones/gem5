#!/bin/bash
# Rebuttal IPC-relative graph: EDF (explicit-data-forwarding) at L1 latency 4
# with 10M warmup.
#
# Prerequisites:
...
#
# Configuration:
#   broadcastMax=12, dependentsThreshold=-1  (baseline IQ broadcast limits)
#   numDmtSlots=2 (default on the explicit-data-forwarding branch)
#   L1 latency = 4
...
OUTPUT_DIR="${GEM5_ROOT}/runs/output/micro26/rebuttal/cache_lat/edf/${L1_LATENCY}"
SLURM_SCRIPT="${GEM5_ROOT}/slurm/single_simpoint_job.slurm"

mkdir -p "${OUTPUT_DIR}/slurm_logs"

echo "=== Rebuttal EDF (L1 latency ${L1_LATENCY}, warmup 10M) ==="
