#!/bin/bash
# Warmup 50M experiment for Hybrid-WL (hybrid-wl branch).
#
# Configuration:
#   broadcastMax=12, dependentsThreshold=2
#   L1 latency = 1 (default)
#   warmup     = 50M detailed O3, no atomic fast-forward
#
# Output tree:
#   runs/output/micro26/rebuttal/warmup/hybrid-wl/50M/
...
OUTPUT_DIR="${GEM5_ROOT}/runs/output/micro26/rebuttal/warmup/hybrid-wl/50M"
SLURM_SCRIPT="${GEM5_ROOT}/slurm/single_simpoint_job.slurm"

mkdir -p "${OUTPUT_DIR}/slurm_logs"

echo "=== Warmup Hybrid-WL (L1 latency ${L1_LATENCY}, warmup 50M) ==="
