...
IPC change (in %) of Sereno relative to the baseline for each core size
(1.0x, 1.5x, 2.0x, 4.0x).
...
Data sources (matched core size per bar group):
  Baseline : runs/output/micro26/rebuttal/big_core/baseline/{1.0x,1.5x,2.0x,4.0x}
  Sereno   : runs/output/micro26/rebuttal/big_core/sereno/{1.0x,1.5x,2.0x,4.0x}
...
Output: runs/output/micro26/rebuttal/big_core/ipc_relative.tex
"""
...
BASELINE_ROOT = BIG_CORE_DIR / "baseline"
SERENO_ROOT = BIG_CORE_DIR / "sereno"

CORE_SIZES = ["1.0x", "1.5x", "2.0x", "4.0x"]

OUTPUT_TEX = BIG_CORE_DIR / "ipc_relative.tex"

INSTS_STAT = "simInsts"
CYCLES_STAT = "system.cpu.numCycles"

# One style entry per core size (1.0x, 1.5x, 2.0x, 4.0x).
BAR_COLORS: list[tuple[str, int, int, int]] = [
    ("clrCore1x", 31, 119, 180),    # tableau blue
    ("clrCore15x", 255, 127, 14),   # tableau orange
    ("clrCore2x", 44, 160, 44),     # tableau green
    ("clrCore4x", 214, 39, 40),     # tableau red
]
...
def generate_tikz(
    baseline_by_core: dict[str, dict[str, float]],
    sereno_by_core: dict[str, dict[str, float]],
    output_path: Path,
) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)

    all_benchmarks = sorted(
        {bm for data in sereno_by_core.values() for bm in data}
        & {bm for data in baseline_by_core.values() for bm in data}
    )
    ...
    for core_size in CORE_SIZES:
        baseline_ipc = baseline_by_core.get(core_size, {})
        sereno_ipc = sereno_by_core.get(core_size, {})
    ...
    for core_size in CORE_SIZES:
        pct = pct_data.get(core_size, {})
        vals_bm = [pct.get(bm, float("nan")) for bm in all_benchmarks]
        vals = vals_bm + [ews_pct.get(core_size, float("nan"))]
        coord_strs = [
            f"({label}, {v:.6f})"
            for label, v in zip(x_labels, vals)
            if v == v
        ]
        coords[core_size] = coord_strs
    ...
    for i, core_size in enumerate(CORE_SIZES):
        ...
        addplot_lines.append(
            ...
            f"        {coord_body}\n"
            f"    }};\n"
            f"    \\addlegendentry{{Core {core_size}}}"
        )
...
def main() -> None:
    ...
    for core_size in CORE_SIZES:
        baseline_dir = BASELINE_ROOT / core_size
        sereno_dir = SERENO_ROOT / core_size

        if not baseline_dir.exists():
            print(f"SKIP baseline core {core_size}: directory not found ({baseline_dir})")
            continue
        if not sereno_dir.exists():
            print(f"SKIP sereno core {core_size}: directory not found ({sereno_dir})")
            continue

        print(f"Collecting IPC for baseline core {core_size} …")
        baseline_ipc = collect_ipc(baseline_dir)
        print(f"  → {len(baseline_ipc)} benchmarks found")

        print(f"Collecting IPC for sereno core {core_size} …")
        sereno_ipc = collect_ipc(sereno_dir)
        print(f"  → {len(sereno_ipc)} benchmarks found")

        baseline_by_core[core_size] = baseline_ipc
        sereno_by_core[core_size] = sereno_ipc

    if not baseline_by_core or not sereno_by_core:
    ...
    header = f"{'Benchmark':<25}" + "".join(f"{core:>14}" for core in CORE_SIZES)
    print("\n" + header)
    print("-" * len(header))

    sereno_ipc_lists: dict[str, list[float]] = {core: [] for core in CORE_SIZES}
    baseline_ipc_lists: dict[str, list[float]] = {core: [] for core in CORE_SIZES}

    for bm in all_bms:
        row = f"{bm:<25}"
        for core_size in CORE_SIZES:
            base = baseline_by_core.get(core_size, {}).get(bm, float("nan"))
            val = sereno_by_core.get(core_size, {}).get(bm, float("nan"))
            pct = (
                (val / base - 1.0) * 100.0
            ...
            row += f"{pct:>13.2f}%"
            if val == val:
                sereno_ipc_lists[core_size].append(val)
            if base == base and base > 0:
                baseline_ipc_lists[core_size].append(base)
        print(row)

    ews_row = f"{'Harmonic Mean':<25}"
    for core_size in CORE_SIZES:
        hm_sereno = harmonic_mean(sereno_ipc_lists[core_size])
        hm_baseline = harmonic_mean(baseline_ipc_lists[core_size])
        ews_pct = (
            (hm_sereno / hm_baseline - 1.0) * 100.0 if hm_baseline > 0 else float("nan")
        )
    ...
    generate_tikz(baseline_by_core, sereno_by_core, OUTPUT_TEX)
