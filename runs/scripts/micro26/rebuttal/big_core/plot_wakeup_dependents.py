...
Read from:
  runs/output/micro26/rebuttal/big_core/baseline/4.0x/width8/{benchmark}/{simpoint}/m5out/stats.txt
...
Output:
  runs/output/micro26/rebuttal/big_core/baseline/4.0x/graphs/wakeup_dependents.tex
"""
...
DATA_DIR = BIG_CORE_DIR / "baseline/4.0x/width8"
OUTPUT_TEX = BIG_CORE_DIR / "baseline/4.0x/graphs/wakeup_dependents.tex"
...
def collect_data(base_dir: Path, debug_bm: str = "") -> dict[str, dict[str, float]]:
    ...
    if debug_bm and debug_bm in weighted:
        ...
        print("  weighted sums   : " + "  ".join(
            f"{s}={wm[k]:.2f}" for s, k in zip(short, STAT_KEYS)
        ))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot wakeup dependents distribution for big-core 4.0x baseline.",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        ...
        help="Root directory to search for stats.txt "
             "(default: big_core/baseline/4.0x/width8)",
    )
    parser.add_argument(
        "--output",
        type=Path,
        ...
        help="Output .tex file path "
             "(default: big_core/baseline/4.0x/graphs/wakeup_dependents.tex)",
    )
    parser.add_argument(
        "--debug",
        metavar="BENCHMARK",
