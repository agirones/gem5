...
in the cache-lat sensibility analysis (L1 latency = 4).

  X-axis  : dependentsThreshold values (−1, 0, 1, 2)
  Bars    : one bar group per x-tick, with 3 bars for broadcastMax 1–3
  Y-axis  : harmonic mean IPC across all benchmarks (weighted per simpoint)

Data source
-----------
  system.cpu.commitStats0.numInsts / system.cpu.numCycles

Read from:
  runs/output/micro26/rebuttal/cache_lat/sensibility_analysis/
    bmax{N}_dt{M}/width8/{benchmark}/{simpoint}/m5out/stats.txt

Normalized against baseline at L1 latency 4:
  runs/output/micro26/rebuttal/cache_lat/baseline/4
...
Output
------
  runs/output/micro26/rebuttal/cache_lat/sensibility_analysis/ipc_sensitivity.tex
"""
...
SENS_DIR = ROOT / "runs/output/micro26/rebuttal/cache_lat/sensibility_analysis"
OUTPUT_TEX = SENS_DIR / "ipc_sensitivity.tex"

INSTS_STAT  = "system.cpu.commitStats0.numInsts"
CYCLES_STAT = "system.cpu.numCycles"

BASELINE_DIR = ROOT / "runs/output/micro26/rebuttal/cache_lat/baseline/4"

BROADCAST_MAX_VALUES        = list(range(1, 4))         # 1 … 3
DEPENDENTS_THRESHOLD_VALUES = list(range(-1, 3))        # -1 … 2
...
DEBUG_BENCHMARK: Optional[str] = None


# ---------------------------------------------------------------------------
# Visual style: one entry per broadcastMax value (1–3)
# ---------------------------------------------------------------------------
# (color_name, R, G, B, pgfplots_pattern | None)
BAR_STYLES: list[tuple[str, int, int, int, Optional[str]]] = [
    ("clrBmax1",  230, 159, 0, "north east lines"),
    ("clrBmax2",  86, 180, 233, None),
    ("clrBmax3",  0, 158, 115, "crosshatch"),
]
...
def generate_tex(data: dict[tuple[int, int], Optional[float]], output_path: Path) -> None:
    ...
    for bmax, style in zip(BROADCAST_MAX_VALUES, BAR_STYLES):
    ...
    tex = (
        ...
        r"        font=\normalsize," "\n"
        r"        cells={anchor=west}," "\n"
        f"        legend columns={len(BROADCAST_MAX_VALUES)},\n"
        r"        column sep=4pt," "\n"
        r"    }," "\n"
        r"]" "\n"
        "\n"
