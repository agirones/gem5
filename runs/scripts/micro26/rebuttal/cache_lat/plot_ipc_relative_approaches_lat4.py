...
Output: runs/output/micro26/rebuttal/cache_lat/ipc_relative_approaches_lat4.tex
"""
...
OUTPUT_TEX = CACHE_LAT_DIR / "ipc_relative_approaches_lat4.tex"
...
def _compute_y_axis(values: list[float], padding: float = 0.0) -> tuple[float, float, list[int]]:
    """
    Return (ymin, ymax, yticks) that fit all *values* with a small margin.

    Limits are rounded outward to multiples of 10; ticks are placed every 10 %.
    """
    if not values:
    ...
    vmin = min(values)
    vmax = max(values)
    ymin = math.floor((min(vmin, 0.0) - padding) / 10.0) * 10.0
    ymax = math.ceil((max(vmax, 0.0) + padding) / 10.0) * 10.0
...
def generate_tikz(
    ...
    all_pct_values = [
        v
        for pct in pct_data.values()
        for v in pct.values()
    ] + list(ews_pct.values())
    ymin, ymax, yticks = _compute_y_axis(all_pct_values)
...
def main() -> None:
    ...
    for name, base_dir in APPROACHES.items():
        if not base_dir.exists():
            ...
            continue
        print(f"Collecting IPC for {name} (L1 latency {L1_LATENCY}) …")
