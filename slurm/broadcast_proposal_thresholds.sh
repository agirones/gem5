...
OUTPUT_DIR="/cluster/home/andreug/research/EECS-NTNU/gem5-NTNU/runs/output/wakeupstats/japanese_fast_no_dest_no_wake_up/"
export OUTPUT_DIR

for MAX_BROADCASTS in $(seq 1 4); do
    for MAX_POINTERS in $(seq 1 2); do
        ...
        if [ "$MAX_BROADCASTS" -eq 1 ] && [ "$MAX_POINTERS" -eq 1 ]; then
            ...
            MODIFY_CODE_JOB_ID=$(sbatch --parsable -D "${EXP_OUTPUT_DIR}" "${SLURM_DIR}/modify_broadcast_and_pointers.slurm" "${MAX_BROADCASTS}" "${MAX_POINTERS}")
        else
            echo "Submitting code modification job with a dependency on ${RUN_JOB_ID}"
            MODIFY_CODE_JOB_ID=$(sbatch --parsable -d "afterany:${RUN_JOB_ID}" -D "${EXP_OUTPUT_DIR}" "${SLURM_DIR}/modify_broadcast_and_pointers.slurm" "${MAX_BROADCASTS}" "${MAX_POINTERS}")
        fi
